...
- **Cards** (Sep 2026): the commissioner deals TCG-style cards (`cards` table) to owners; only the owner sees their hand (`my_cards`), plays one per tournament before lock (`play_card`/`unplay_card`), and plays reveal at lock (`tournament_cards`). **All points flow through `scored_points(tournament_id)`** — winnings × multiplier, then cards applied in `played_at` order: `multiply {x}`, `flat {amount}`, `duel` (higher raw winnings → ×2 base, loser 0; tie no-op), `steal {pct}`, `swap`, `shield` (nullifies duel/steal/swap aimed at the holder), `mulligan` (lets `submit_pick` reuse a golfer that week), `custom` (text only). `tournament_board`/`standings`/`season_picks`/`my_picks` all read from it and carry a `note` explaining the math. Adding an effect = add it to the `check` constraint, the `case` in `scored_points`, and `EFFECTS` in index.html (label/param/target/summary). Card art is stored as a data URL resized to ≤640px in the browser (`dcImage` handler) — no storage bucket.

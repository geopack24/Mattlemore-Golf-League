...
happened (e.g. "Ryder Die: beat Mike (×2)"). At most three cards per owner per tournament (unless a card exempts itself), and each owner has **3 tees** a week to spend: every card costs 0–3 tees to play. When a tournament ends everyone gets a **booster pack** of 5 random library cards (odds in `RULES.md`). Cards whose type line says **Instant** can be played mid-tournament until 8 PM ET on day 3, or later as a response to an Instant played on you — see `RULES.md`. Shields cancel duels, steals and
